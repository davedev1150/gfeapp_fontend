...
const StockEditPage: React.FC<any> = () => {
  ...
  const stockEditReducer = useSelector(
    (state: RootState) => state.stockEditReducer
  );
  ...
  const showPreviewImage = (values: any) => {
    if (values.file_obj) {
      ...
      return (
        <img
          src={`${imageUrl}/image/${values.image}`}
          style={{ height: 150 }}
        />
      );
    }
      ...
      <Form>
        <Card>
          <CardContent sx={{ padding: 4 }}>
            ...
            <div>
              <img
                src={`${process.env.PUBLIC_URL}/image/ic_photo.png`}
  ...
  const initialValues: Product = { name: "Loading...", stock: 0 };
    ...
    <Box>
      <Formik
        ...
        onSubmit={(values, { setSubmitting }) => {
          ...
          dispatch(stockEditActions.updateProduct(values,formData));
          setSubmitting(false);
        }}
      >
